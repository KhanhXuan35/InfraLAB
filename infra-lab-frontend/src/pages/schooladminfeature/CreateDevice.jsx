...
const DuplicateDeviceModal = ({ visible, info, type, onConfirm, onCancel }) => (
  <Modal
    title={
    ...
    onOk={onConfirm}
    onCancel={onCancel}
    okText={type === "other_category" ? "Xem chi tiết thiết bị" : "Thêm vào thiết bị này"}
    cancelText="Hủy"
        ...
        <Card size="small" style={{ marginTop: 16, background: "#fafafa" }}>
          <Row gutter={[16, 8]}>
            ...
            <Col span={12}>
              ...
              <div style={{ marginTop: 4 }}>
                <Text strong>{info.category?.name || "N/A"}</Text>
              </div>
            </Col>
            <Col span={12}>
              <Text type="secondary">Tổng số lượng:</Text>
              <div style={{ marginTop: 4 }}>
                <Text strong>{info.totalInstances} chiếc</Text>
              </div>
            </Col>
            <Col span={12}>
              <Text type="secondary">Phân bổ theo vị trí:</Text>
              <div style={{ marginTop: 4 }}>
                {Object.entries(info.instancesByLocation || {}).map(([loc, count]) => (
                  <div key={loc}>
                    <Text>
...
const CreateDevice = () => {
  ...
  const buildCategoryOptions = (list, searchText) => {
    if (!searchText) {
      return list.map((cat) => ({ value: cat.name, label: cat.name }));
    }

    const matched = list.filter((cat) =>
      cat.name.toLowerCase().includes(searchText.toLowerCase())
    );
    const options = matched.map((cat) => ({ value: cat.name, label: cat.name }));
  ...
  useEffect(() => {
    setCategoryOptions(buildCategoryOptions(categories, categorySearchValue));
  }, [categories, categorySearchValue]); // cập nhật options khi thêm category mới

  // Xử lý khi search category
  const handleCategorySearch = (value) => {
    setCategorySearchValue(value);
    setCategoryOptions(buildCategoryOptions(categories, value));
  };
    ...
    if (option.isCreate) {
      try {
        ...
        if (res.success && res.data) {
          ...
          setCategoryOptions(buildCategoryOptions([...categories, res.data], newCategoryName));
  ...
  const handleDuplicateFromResponse = (data) => {
    if (!data?.duplicate || !data?.existingDevice) return false;
    setDuplicateDeviceInfo(data.existingDevice);
    setDuplicateType(data.duplicateType || "same_category");
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      if (res.success) {
        ...
        if (!handleDuplicateFromResponse(res)) {
      ...
      if (!handleDuplicateFromResponse(error)) {
